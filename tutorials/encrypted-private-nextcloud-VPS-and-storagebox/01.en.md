...
In the AIO interface, tick **"Full text search"** among the optional containers and apply. AIO creates the `nextcloud_aio_elasticsearch` docker volume and the `nextcloud-aio-fulltextsearch` container. Then stop the stack from the AIO interface before continuing — the volume must be idle for the next part.
...
Then start the stack from the AIO interface and check:
