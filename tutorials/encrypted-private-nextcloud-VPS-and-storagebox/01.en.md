...
The failure this guards against is not "Nextcloud cannot reach the files" — it is **"Nextcloud can see the directory, but the data is not behind it"**. A missing bind mount leaves an ordinary, readable, *empty* directory at `/mnt/ncdata/<user>/files`. Nothing errors. Nextcloud simply believes the user has no files.
...
Once you click on `Set backup location`, you should see an SSH key in the AIO interface. If not, click `Create backup`. Note that this will turn off all containers. You will get a warning like this:
...
This is the deliberate part: security updates get *installed* automatically, but anything needing a reboot (kernel, glibc, systemd) stays *inactive* until you reboot and unlock manually. The next piece makes sure you find out when that's the case.
...
# unattended-upgrades installed a new kernel / glibc / systemd).
#
...
occ(){ docker exec --user www-data "$CT" php occ "$@" 2>/dev/null; }
...
```ini
[Unit]
Description=Notify (Nextcloud) when a reboot is pending (kernel/glibc security updates)
...
```bash
find /mnt/myshare -depth | grep -P '[\x{F000}-\x{F0FF}]' | head
```
...
Nextcloud's full-text search indexes the *contents* of your files, not just their names. It needs Elasticsearch, and the index is big: roughly **50 KB per document**, so ~300,000 files produce a **~15 GB** index. That is usually far too much for the small LUKS-encrypted VPS disk you are keeping your keys and database on.
...
In the AIO interface, stop the containers, then tick **"Full text search"** among the optional containers and apply. AIO creates the `nextcloud_aio_elasticsearch` Docker volume and the `nextcloud-aio-fulltextsearch` container. Then stop the stack from the AIO interface before continuing — the volume must be idle for the next part.

### Create the encrypted container on the Storage Box

```bash
# A 25 GB container file. Do NOT use fallocate/truncate: on CIFS they produce a sparse
...
- **It takes a long time.** With `O_DIRECT` (which removes the write cache — the price of not corrupting) expect roughly **2,700 documents/hour**. 300,000 files is therefore about **4-5 days**. Run it in `tmux` or `screen`.
...
Then call it alongside each `notify`, most importantly the one that announces maintenance mode:
...
```bash
notify "$label maintenance mode ON" "..."
brrr "$label maintenance mode ON" "..." "critical"
```
...
If you already built this and want the fixes without rebuilding, do them in this order. Only the first one is urgent.
...
1. **Check whether your SMB mount is `soft` — it probably is.**
   ...
   A reboot is the honest way to do this. Remounting by hand means releasing every bind mount and stopping Docker first, and getting that half-right is how you end up running `files:scan` against empty directories. Verify after the reboot with the same `findmnt` command.

2. **Check whether anything was damaged while the mount was soft.**
...
3. **Check what your firewall actually covers.**
...
4. **Optional additions.**
   ...
   Step 16 (the capacity and availability guard) and Step 15 (encrypted Elasticsearch volume) are self-contained — follow them as written on a running system. Step 16 in particular pairs with the `hard` mount from item 1: together they cover both "storage is gone" and "storage is full", which is why they are worth having as a pair.

5. **Still on the original layout?**
...
- Added **Step 15**: optional full-text search with the Elasticsearch index in a LUKS container **on the Storage Box**, so a ~15 GB index does not have to fit on the small local disk. Same key-separation model — the key stays on the VPS.
